...
int main(int argc, char** argv) {
  ...
  MakeBuffer(count * 7 * sizeof(float), out_buf, out_mem);
  ...
  void* out_mapped;
  CHK(vkMapMemory(device, out_mem, 0, count * 7 * sizeof(float), 0, &out_mapped));
  const float* out = static_cast<const float*>(out_mapped);
  printf("input,sin,cos,sqrt,inversesqrt,trunc,floor,fract\n");
  for (uint32_t i = 0; i < count; ++i) {
    printf("%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g\n", inputs[i],
           out[i * 7 + 0], out[i * 7 + 1], out[i * 7 + 2], out[i * 7 + 3],
           out[i * 7 + 4], out[i * 7 + 5], out[i * 7 + 6]);
  }
