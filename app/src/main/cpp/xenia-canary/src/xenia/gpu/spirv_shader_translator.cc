...
std::vector<uint8_t> SpirvShaderTranslator::CompleteTranslation() {
  ...
  } else if (IsSpirvComputeShader()) {
    // Memexport emulated via a compute shader, because on tiled/binning GPUs
    // (e.g. Adreno) memory stores from the vertex pipeline are unreliable - the
    // vertex shader may be run in a position-only binning pass, so its stores
    // are stripped or duplicated. A compute dispatch runs each guest vertex
    // exactly once. Local size is 1 so the host dispatch can request exactly the
    // guest vertex count without a bounds check (the invocation ID is the vertex
    // index directly).
    execution_model = spv::ExecutionModelGLCompute;
    builder_->addExecutionMode(function_main_, spv::ExecutionModeLocalSize, 1, 1,
                               1);
  } else {
...
void SpirvShaderTranslator::StartVertexOrTessEvalShaderBeforeMain() {
  ...
  if (IsSpirvTessEvalShader()) {
    ...
    // Compute-emulated memexport: the guest vertex index comes from the compute
    // global invocation ID (local size is 1, so invocation .x == vertex index).
    input_global_invocation_id_ =
        builder_->createVariable(spv::NoPrecision, spv::StorageClassInput,
                                 type_uint3_, "gl_GlobalInvocationID");
    builder_->addDecoration(input_global_invocation_id_,
  ...
  // A compute shader emulating memexport has no rasterization outputs
  // (interpolators, point parameters, gl_Position) - it only writes to memory.
...
void SpirvShaderTranslator::StartVertexOrTessEvalShaderInMain() {
  ...
  // Zero the interpolators (a compute memexport shader has no interpolator
  // outputs to zero).
  if (!IsSpirvComputeShader()) {
    uint32_t interpolators_remaining = GetModificationInterpolatorMask();
    uint32_t interpolator_index;
  ...
  if (register_count()) {
    ...
    if (IsSpirvVertexShader()) {
      // For compute-emulated memexport, the guest vertex index is the compute
      // global invocation ID's X component (local size is 1). For a regular
      // vertex shader, it's gl_VertexIndex.
      spv::Id vertex_index;
      if (IsSpirvComputeShader()) {
        vertex_index = builder_->createCompositeExtract(
...
void SpirvShaderTranslator::CompleteVertexOrTessEvalShaderInMain() {
  // A compute shader emulating memexport produces no position or point-size
  // outputs - it only writes to memory during the guest shader body.
  if (IsSpirvComputeShader()) {
    return;
  }
  id_vector_temp_.clear();
...
void SpirvShaderTranslator::StoreResult(const InstructionResult& result,
  ...
  switch (result.storage_target) {
    ...
    case InstructionStorageTarget::kPosition: {
      assert_true(is_vertex_shader());
      // A compute memexport shader has no gl_Position output - drop the write
      // (target_pointer stays NoResult, handled below).
      if (IsSpirvComputeShader()) {
        break;
      }
      id_vector_temp_util_.clear();
