...
VkImageView VulkanTextureCache::GetActiveBindingOrNullImageView(
  ...
  if (binding && AreDimensionsCompatible(dimension, binding->key.dimension)) {
    // Check for 3D texture sampled as 2D (e.g. stacked/lightmap textures).
    ...
    if (force_special_view) {
      // Respect swizzled_signs from the fetch constant, not just the shader's
      // request, and use the separate signed texture if the format requires
      // it.
...
VulkanTextureCache::SamplerParameters VulkanTextureCache::GetSamplerParameters(
  ...
  // Apply anisotropic override, but only for mipmapped textures that are
  // already using bilinear/trilinear filtering.
...
bool VulkanTextureCache::LoadTextureDataFromResidentMemoryImpl(Texture& texture,
  ...
  // For 3D-as-2D wrappers, the host image is 2D but reading from guest memory
  // still needs 3D tiling math.
