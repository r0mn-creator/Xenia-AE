...
DEFINE_bool(
    ...
    // NOTE (Canary AE): XenDroid ships this ON. Ported here 2026-08-13 and it
    // REGRESSED badly - Halo 3 stalled on the legal screen at 1.6 FPS and the
    // PM4 stream collapsed from ~4.37M packets to ~8K. Defaulted OFF pending
    // investigation; the likely cause is that XenDroid's version depends on
    // its preemption subsystem (OPCODE_CHECK_PREEMPT / PreemptCheckInjection-
    // Pass) which is NOT ported here. See docs/HALO3_VISTA_46_VS_64.md s34.
